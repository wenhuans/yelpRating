...
tmp = data['sentiment_score'].values
...
data_all = data[['text','stars','sentiment_score']]
# data_all['stars'].hist()
# data_all.head()
# plt.show()
...
print('******************size check sentiment --> x_train',data_train_one_hot['sentiment_score'].values.reshape((train_len,1)).shape, x_train.shape)
print('******************************* check here y labels', y[0:4])
print(' senti check',data_train_one_hot['sentiment_score'].values.reshape((train_len,1))[0:4])
...
auxiliary_input = Input(shape=(1,), name='aux_input') # todo
x_conc = keras.layers.concatenate([conc, auxiliary_input]) # todo
outp = Dense(5, activation = 'sigmoid')(x_conc)
...
model.fit([x_train,data_train_one_hot['sentiment_score'].values.reshape((train_len,1))], y, batch_size = 512, epochs = 20, validation_split = .1,
          callbacks=[earlystop, checkpoint])

model.save('yelp_nn_mode_include_sentiment_score_test.h5')
print('model saved')

import pickle
# Saving the objects:
with open('objs_tf_include_sentiment_score_test.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
    pickle.dump([x_train, x_test, y, data_test_one_hot], f)
print('data saved to file')

print('model evaluation...')
y_test = model.predict([x_test,data_test_one_hot['sentiment_score'].values.reshape((test_len,1))], batch_size=1024, verbose = 1)
tmp= model.evaluate([x_test, data_test_one_hot['sentiment_score'].values.reshape((test_len,1))], data_test_one_hot[class_names].values, verbose = 1, batch_size=1024)
